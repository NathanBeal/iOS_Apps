...
class TeamsPage3: SKScene
{
    ...
    var Page4Teams:[String] = ["None","None","None","None","None","None","None","None"]
    ...
    override func didMove(to view: SKView)
    {
        ...
        {
            var bound = 0
            if DivisionTeams.count >= 32
            {
                bound = 32
            }else{
                bound = DivisionTeams.count
            }
            for index in 24...bound-1
            {
                //Page4Teams.append(DivisionTeams[index])
                Page4Teams[index-24] = (DivisionTeams[index])
            }
        }
    ...
    func moveToTeamsPage2()
    {
        let sceneToMoveTo = TeamsPage2(size: self.size)
        sceneToMoveTo.scaleMode = self.scaleMode
        let myTransition = SKTransition.fade(withDuration:  0.0)
        self.view!.presentScene(sceneToMoveTo, transition: myTransition)
    }
    
    func moveToTeamsPage4()
    {
        let sceneToMoveTo = TeamsPage4(size: self.size)
        sceneToMoveTo.scaleMode = self.scaleMode
        let myTransition = SKTransition.fade(withDuration:  0.0)
        self.view!.presentScene(sceneToMoveTo, transition: myTransition)
    }
