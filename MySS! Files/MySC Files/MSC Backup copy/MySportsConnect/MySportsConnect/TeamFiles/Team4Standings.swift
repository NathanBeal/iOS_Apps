//
//  Team4StandingsPage.swift
//  MySportsConnect
//
//  Created by ADAMLANE on 7/23/19.
...
class Team4Standings: SKScene
{
    ...
    {
        let temp = String(defaults.string(forKey: "Team4Txt")!)
        ...
        //Team Labeling
        let Team = defaults.string(forKey: "Team4")
        ...
        contactDetails.text = "There are no standings available"
    ...
    {
        let sceneToMoveTo = Team4ContactPage(size: self.size)
        sceneToMoveTo.scaleMode = self.scaleMode
        let myTransition = SKTransition.fade(withDuration:  0.0)
        self.view!.presentScene(sceneToMoveTo, transition: myTransition)
    }
