...
class gamePlay: SKScene
{
    ...
    //Menu Stuff
    /*
    let menuIcon = SKSpriteNode(imageNamed: "menuIcon")
    let menu = SKSpriteNode(imageNamed: "menu")
    let menuBO = SKSpriteNode(imageNamed: "menuBackdropOverlay")
    ...
    //For statistics
    var statisticsLabel1 = SKLabelNode()
    var statisticsLabel2 = SKLabelNode()
    var statisticsLabel3 = SKLabelNode()
    var statisticsLabel4 = SKLabelNode()
    var statisticsLabel5 = SKLabelNode()
    
    var statsLabel1 = SKLabelNode()
    var statsLabel2 = SKLabelNode()
    var statsLabel3 = SKLabelNode()
    var statsLabel4 = SKLabelNode()
    ...
    //ScreenDown
    let handRanks = SKLabelNode()
    let royalFlush = SKLabelNode()
    let royalFlushExp = SKLabelNode()
    let straightFlush = SKLabelNode()
    let straightFlushExp = SKLabelNode()
    let fourKind = SKLabelNode()
    let fourKindExp = SKLabelNode()
    let fullHouse = SKLabelNode()
    let fullHouseExp = SKLabelNode()
    let flush = SKLabelNode()
    let flushExp = SKLabelNode()
    let straight = SKLabelNode()
    let straightExp = SKLabelNode()
    let threeKind = SKLabelNode()
    let threeKindExp = SKLabelNode()
    let twoPair = SKLabelNode()
    let twoPairExp = SKLabelNode()
    let aPair = SKLabelNode()
    let aPairExp = SKLabelNode()
    let highCard = SKLabelNode()
    let highCardExp = SKLabelNode()
    ...
    override func didMove(to view: SKView)
    {
        ...
        Directions.text = "tap on overlay to select the flop"
        Directions.fontSize = CGFloat(38 * Scaler)
        Directions.fontColor = SKColor.white
        Directions.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.center
        ...
        settingGameMode.position = CGPoint(x: self.size.width/2, y: 0.62 * self.size.height)
        ...
        gameManual = CGRect(x: 2.666 * self.size.width/10, y: settingsMenuRect.midY, width: 2 * self.size.width/10, height: 0.8 * self.size.height/10)
        gameSimulated = CGRect(x: 5.333 * self.size.width/10, y: settingsMenuRect.midY, width: 2 * self.size.width/10, height: 0.8 * self.size.height/10)
        ...
        {
            gameModeOverlay.position = CGPoint(x: 3.666 * self.size.width/10, y: settingsMenuRect.midY + 0.4 * self.size.height/10)
        ...
        {
            gameModeOverlay.position = CGPoint(x: 6.333 * self.size.width/10, y: settingsMenuRect.midY + 0.4 * self.size.height/10)
            //manualPlay.fontColor = SKColor.black
        ...
        {
            flopDone = true
            Directions.text = "tap on overlay to select the turn"
        }
        
        if defaults.string(forKey: "Turn") != "None"
        {
            turnDone = true
            Directions.text = "tap on overlay to select the river"
        }
        ...
        if defaults.double(forKey: "numberOfPlayers") == 6.0
        {
            ...
            self.addChild(extraCard3)
            extraCard4.setScale(CGFloat(0.165 * Scaler))
            extraCard4.position = CGPoint(x: 6.15 * self.size.width/8, y: 10 * self.size.height/10)
            extraCard4.zPosition = 2
            extraCard4.zRotation = CGFloat(Double.pi)
            self.addChild(extraCard4)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        for touch: AnyObject in touches
        {
            ...
            if settingDisplay == true
            {
                ...
                //GameMode
                if (pointOfTouch.x > 3.666 * self.size.width/10 - gameManual.width/2 && pointOfTouch.x <  3.666 * self.size.width/10 + gameManual.width/2) && (pointOfTouch.y > settingsMenuRect.midY - gameManual.height/2 && pointOfTouch.y < settingsMenuRect.midY + gameManual.height/2)
                {
                    gameModeOverlay.position = CGPoint(x: 3.666 * self.size.width/10, y: settingsMenuRect.midY + 0.4 * self.size.height/10)
                    defaults.set(0, forKey: "tempGameMode")
                }
                if (pointOfTouch.x > 6.333 * self.size.width/10 - gameManual.width/2 && pointOfTouch.x < 6.333 * self.size.width/10 + gameManual.width/2) && (pointOfTouch.y > settingsMenuRect.midY - gameManual.height/2 && pointOfTouch.y < settingsMenuRect.midY + gameManual.height/2)
                {
                    gameModeOverlay.position = CGPoint(x: 6.333 * self.size.width/10, y: settingsMenuRect.midY + 0.4 * self.size.height/10)
                ...
                if (flopDone == true) && (defaults.string(forKey: "Turn") == "None")
                {
                    ...
                    if ((pointOfTouch.x > (7.5 * self.size.width/16 - flopGuide.size.width/2) && pointOfTouch.x < (7.5 * self.size.width/16 + flopGuide.size.width/2)) && (pointOfTouch.y > (3.25 * self.size.height/10 - flopGuide.size.height/2) && pointOfTouch.y < (3.25 * self.size.height/10 + flopGuide.size.height/2)))
                    {
                        ...
                        let switchScene = SKAction.run(moveToGameSceneFlop)
                        let sequence = SKAction.sequence([switchScene])
                        self.run(sequence)
                    }
    ...
    func riverStatistics()      //River -> Turn
    {
        ...
        maxCycle = 0
        let rank = handRanking(handSize: 6)
    ...
    {
        maxCycle = 0
        let rank = handRanking(handSize: 7)
        bestHand(number: rank)
        self.addChild(betterHandStat)
    }
