...
class GameSceneRiver: SKScene
{
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        for touch: AnyObject in touches
        {
            ...
            if (pointOfTouch.x > (1.5 * self.size.width/6 - cardWidth/2) && pointOfTouch.x < (1.5 * self.size.width/6 + cardWidth/2)) && (pointOfTouch.y > (self.size.height/2 - cardHeight/2) && pointOfTouch.y < (self.size.height/2 + cardHeight/2))
            {
                ...
                let sceneToMoveTo = heartSelectRiver(size: self.size)
            ...
            {
                self.removeAllChildren()
                let sceneToMoveTo = diamondSelectRiver(size: self.size)
                sceneToMoveTo.scaleMode = self.scaleMode
                let myTransition = SKTransition.fade(withDuration:  1.0)
                self.view!.presentScene(sceneToMoveTo, transition: myTransition)
            }
            ...
            {
                self.removeAllChildren()
                let sceneToMoveTo = clubSelectRiver(size: self.size)
                sceneToMoveTo.scaleMode = self.scaleMode
                let myTransition = SKTransition.fade(withDuration:  1.0)
                self.view!.presentScene(sceneToMoveTo, transition: myTransition)
            }
            ...
            {
                self.removeAllChildren()
                let sceneToMoveTo = spadeSelectRiver(size: self.size)
                sceneToMoveTo.scaleMode = self.scaleMode
                let myTransition = SKTransition.fade(withDuration:  1.0)
                self.view!.presentScene(sceneToMoveTo, transition: myTransition)
            }
    ...
    func displayCards()
    {
        ...
        if key1 != "None"
        {
            let temp = SKSpriteNode(imageNamed: defaults.string(forKey: "River")!)
