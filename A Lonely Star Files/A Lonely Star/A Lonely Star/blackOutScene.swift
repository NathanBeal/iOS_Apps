...
class blackOutScene: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)//runs as soon as started
    {
        ...
        let checkHP = SKAction.run(checkPlayerHP)
        let leaveSequence = SKAction.sequence([waitToChangeScene, checkHP])
    ...
    func checkPlayerHP()
    {
        if playerHealth > 0
        {
            self.run(SKAction.run(playerLived))
        }else{
            self.run(SKAction.run(playerDied))
        }
    }
}
